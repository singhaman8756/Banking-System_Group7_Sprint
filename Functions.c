...

{
	...
	while(1)
	{
		...
		{
			if(!check(name[i])) //check() function is used to check whether the name is in number or alphabet, if number then it return 0
			{
				flag=1;
				break;
			}
		...
		while(1)
		{
			...
			if(aadhar<100000000000|| an>999999999999) //Checking the length of Aadhar number as it should be of 12 digits only else it will print invalid message
			{
			...
			sprintf(aadhar_no,"%d",aadhar); //stores integer in string

//	printf("\nEnter your password: \n");
//	scanf("%s",password);
        char *ptr=getpassword(); //storing password in pointer
	...
	new_acc->aadhar_no=atoi(aadhar_no); //Typecasting and storing it in pointer
	strcpy(new_acc->password,password); //storing password in pointer of customer structure
